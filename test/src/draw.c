...
void draw_tour(cairo_surface_t *surface, instance_t *instance, tour_t *tour) {
	...
	int width = cairo_image_surface_get_width(surface), height = cairo_image_surface_get_height(surface);
	int sizing = MIN(width, height);
...
void draw_instance_tour(instance_t *inst, tour_t *tour, char *filename) {
	cairo_surface_t *surf = cairo_image_surface_create(CAIRO_FORMAT_RGB24, IMG_WIDTH, IMG_HEIGHT);
	{
